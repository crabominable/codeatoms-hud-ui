...
$(document).ready(function(){
	window.addEventListener("message",function(event){
		if (event["data"]["progress"] == true){
			...
			if($("#progressBackground").css("display") === "flex"){
				...
				$("#progressBackground").css("display","flex");
				$("#progressDisplay").css("stroke-dashoffset","100");
			}
		...
		if (lastHealth !== event["data"]["health"]){
			...
			if (event["data"]["health"] <= 1){
				...
				$(".healthDisplay").css("stroke-dashoffset",100 - event["data"]["health"] / 3);
			}
		}
		...
		if (lastArmour !== event["data"]["armour"]){
			...
			if (event["data"]["armour"] <= 0){
				if($(".armourBackground").css("display") === "flex"){
					$(".armourBackground").css("display","none");
				}
			} else {
				if($(".armourBackground").css("display") === "none"){
					$(".armourBackground").css("display","flex");
				}
			}
		...
		if (event["data"]["vehicle"] !== undefined){
			if (event["data"]["vehicle"] == true){
				if($(".displayTop").css("display") === "none"){
					...
					$(".displayTop").css("margin","0px");
					$("#displayMiddle").css("margin-bottom","0px")
					$("#displayflexhud").css("width", "20em");
				}
				...
				if (event["data"]["showbelt"] == false){
					...
					if (event["data"]["seatbelt"] == 1){
						$("#seatbelt-icon").attr("src",'seatbelt.png');
					} else {
						$("#seatbelt-icon").attr("src",'seatbelt-norm.png');
					}
				}

				$("#gasoline").html(parseInt(event["data"]["fuel"]));
				$("#mph").html("<s>" + parseInt(event["data"]["speed"] * 2.00934) + "</s> km/h");
			} else {
				if($(".displayTop").css("display") === "flex"){
					$(".displayTop").css("display","none");
					$(".displayTop").css("margin-left","1.4em");
					$("#displayMiddle").css("margin-bottom","4px")
					$("#displayflexhud").css("width", "14em");
				}
			}
		}

		$("#displayMiddle").html(event["data"]["radio"] + "SÃO " + event["data"]["hours"] + " HORAS E " + event["data"]["minutes"] + " MINUTOS");
	});
});